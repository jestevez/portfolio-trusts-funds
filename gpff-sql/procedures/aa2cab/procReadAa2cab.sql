...
--DROP  PROCEDURE  GPSQLWEB.procReadAa2cab

CREATE PROCEDURE  GPSQLWEB.procReadAa2cab (
     IN P_AREA varchar(4) ,
...
BEGIN
  ...
  Declare C1 Cursor With Return For stmt;
    ...
    IF P_FIRSTRESULT > 0 AND P_MAXRESULT > 0 THEN
         Set StringSQL = 'SELECT * FROM ( SELECT ROW_NUMBER() OVER() AS ROW__NUMBER, AREA ,NAREA ,USUARIO ,PANTALLA ,FECHA ,LITERAL1 ,LITERAL2 ,NCTA ,NCTAC ,NCTACU ,NCTCHE ,NCTCHA ,NCTBCV ,NCTAFL ,NCTAIS ,NCTGAO ,NCTRIV  FROM GPDATWEB.AA2CAB  WHERE 1=1 ' || WhereClause || ' ) AS LIMITED__TABLE WHERE ROW__NUMBER BETWEEN '|| P_FIRSTRESULT || ' and '|| ((P_MAXRESULT+P_FIRSTRESULT) -1) || SortClause ;
    ELSE
        IF P_MAXRESULT > 0 THEN
            Set StringSQL = 'SELECT  AREA ,NAREA ,USUARIO ,PANTALLA ,FECHA ,LITERAL1 ,LITERAL2 ,NCTA ,NCTAC ,NCTACU ,NCTCHE ,NCTCHA ,NCTBCV ,NCTAFL ,NCTAIS ,NCTGAO ,NCTRIV  FROM  GPDATWEB.AA2CAB WHERE 1=1 ' || WhereClause || SortClause || ' FETCH FIRST '|| P_MAXRESULT ||' ROWS ONLY '; 
        ELSE
            Set StringSQL = 'SELECT  AREA ,NAREA ,USUARIO ,PANTALLA ,FECHA ,LITERAL1 ,LITERAL2 ,NCTA ,NCTAC ,NCTACU ,NCTCHE ,NCTCHA ,NCTBCV ,NCTAFL ,NCTAIS ,NCTGAO ,NCTRIV  FROM  GPDATWEB.AA2CAB WHERE 1=1 ' || WhereClause || SortClause; 
        END IF;
    END IF;

    CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Leer', 'procReadAa2cab', StringSQL);
...
--call GPSQLWEB.procReadAa2cab ('','','','',0,'','','','','','','','','','','','', 1, 10, '', '', '', '')
