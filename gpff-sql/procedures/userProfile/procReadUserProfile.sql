...
--DROP  PROCEDURE  GPSQLWEB.procReadUserProfile

CREATE PROCEDURE  GPSQLWEB.procReadUserProfile (
     IN P_ID INTEGER ,
  ...
  Declare C1 Cursor With Return For stmt;
    ...
    */
    ...
    IF P_FIRSTRESULT > 0 AND P_MAXRESULT > 0 THEN
         Set StringSQL = 'SELECT * FROM ( SELECT ROW_NUMBER() OVER() AS ROW__NUMBER, ID ,USERNAME ,DOCUMENT_NUMBER ,FIRST_NAME ,SECOND_NAME ,LAST_NAME ,SECOND_LAST_NAME ,GENDER ,CIVIL_STATUS ,EMAIL ,PHONE ,MOBILE ,BIRTHDAY ,ENABLED ,DELETED ,CREATED_AT ,CREATED_BY ,UPDATED_AT ,UPDATED_BY  FROM GPSQLWEB.USER_PROFILE  WHERE 1=1 ' || WhereClause || ' ) AS LIMITED__TABLE WHERE ROW__NUMBER BETWEEN '|| P_FIRSTRESULT || ' and '|| ((P_MAXRESULT+P_FIRSTRESULT) -1) || SortClause ;
    ELSE
        IF P_MAXRESULT > 0 THEN
            Set StringSQL = 'SELECT  ID ,USERNAME ,DOCUMENT_NUMBER ,FIRST_NAME ,SECOND_NAME ,LAST_NAME ,SECOND_LAST_NAME ,GENDER ,CIVIL_STATUS ,EMAIL ,PHONE ,MOBILE ,BIRTHDAY ,ENABLED ,DELETED ,CREATED_AT ,CREATED_BY ,UPDATED_AT ,UPDATED_BY  FROM  GPSQLWEB.USER_PROFILE WHERE 1=1 ' || WhereClause || SortClause || ' FETCH FIRST '|| P_MAXRESULT ||' ROWS ONLY '; 
        ELSE
            Set StringSQL = 'SELECT  ID ,USERNAME ,DOCUMENT_NUMBER ,FIRST_NAME ,SECOND_NAME ,LAST_NAME ,SECOND_LAST_NAME ,GENDER ,CIVIL_STATUS ,EMAIL ,PHONE ,MOBILE ,BIRTHDAY ,ENABLED ,DELETED ,CREATED_AT ,CREATED_BY ,UPDATED_AT ,UPDATED_BY  FROM  GPSQLWEB.USER_PROFILE WHERE 1=1 ' || WhereClause || SortClause; 
        END IF;
    END IF;

    CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME1, 'Leer', 'procReadUserProfile', StringSQL);
...
--call GPSQLWEB.procReadUserProfile (0,'','','','','','','','','','','','','','','','','','', 1, 10, '', '', '', '')
