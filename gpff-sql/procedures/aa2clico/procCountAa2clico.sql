...
--DROP  PROCEDURE  GPSQLWEB.procCountAa2clico

CREATE PROCEDURE  GPSQLWEB.procCountAa2clico (
     IN P_CLCCLA BIGINT,
 IN P_CLCCTR INTEGER,
...
-- WHERE CLAUSE
        ...
        IF P_CLCCLA IS NOT NULL AND length(P_CLCCLA)>0 THEN
            SET P_CLCCLA = UPPER(TRIM(P_CLCCLA));
            SET WhereClause = WhereClause || ' AND CLCCLA LIKE ''%'|| P_CLCCLA ||'%'' ';
        END IF;
        ...
        IF P_CLCNUM IS NOT NULL AND length(P_CLCNUM)>0 THEN
            SET P_CLCNUM = UPPER(TRIM(P_CLCNUM));
            SET WhereClause = WhereClause || ' AND CLCNUM LIKE ''%'|| P_CLCNUM ||'%'' ';
        END IF;
    
        
        IF P_CLCANA IS NOT NULL AND length(P_CLCANA)>0 THEN
            SET P_CLCANA = UPPER(TRIM(P_CLCANA));
            SET WhereClause = WhereClause || ' AND CLCANA LIKE ''%'|| P_CLCANA ||'%'' ';
        END IF;
        ...
        IF P_CLCNIF IS NOT NULL AND length(P_CLCNIF)>0 THEN
            ...
            SET WhereClause = WhereClause || ' AND CLCNIF LIKE ''%'|| P_CLCNIF ||'%'' ';
        END IF;
    
        
        IF P_CLCTEL IS NOT NULL AND length(P_CLCTEL)>0 THEN
            SET P_CLCTEL = UPPER(TRIM(P_CLCTEL));
            SET WhereClause = WhereClause || ' AND CLCTEL LIKE ''%'|| P_CLCTEL ||'%'' ';
        END IF;
...
Set StringSQL = 'SELECT COUNT(1) FROM  GPDATWEB.AA2CLICO  WHERE 1=1 ' || WhereClause; 
PREPARE stmt1 FROM StringSQL;
CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Contar', 'procCountAa2clico', StringSQL);
OPEN c1;
    FETCH c1 into total;
CLOSE c1;
...
--call GPSQLWEB.procCountAa2clico(0,0,'','','','','','','',0,'','',0,0,'','',0,'', '', '', '')
