...
--DROP PROCEDURE GPSQLWEB.procDeleteAa2cta

CREATE PROCEDURE GPSQLWEB.procDeleteAa2cta (
    IN P_ROWID BIGINT,
...
SET P_MSGCODE = 9999;
  ...
  IF V_CANT > 1 THEN
	SET P_MSGCODE = 9001;
        ...
        CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Eliminar', 'procDeleteAa2cta', StringSQL);
        RETURN;
  END IF;
  ...
  DELETE FROM GPDATWEB.AA2CTA WHERE RRN(GPDATWEB.AA2CTA) = P_ROWID;
  
  Set StringSQL = 'DELETE FROM GPDATWEB.AA2CTA WHERE RRN(GPDATWEB.AA2CTA) = '''|| P_ROWID || ''';';
  CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Eliminar', 'procDeleteAa2cta', StringSQL);
