...
--DROP  PROCEDURE  GPSQLWEB.procCountCgmaxmin

CREATE PROCEDURE  GPSQLWEB.procCountCgmaxmin (
...
Set StringSQL = 'SELECT COUNT(1) FROM  GPDATWEB.CGMAXMIN  WHERE 1=1 ' || WhereClause; 
PREPARE stmt1 FROM StringSQL;
CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Contar', 'procCountCgmaxmin', StringSQL);
OPEN c1;
    FETCH c1 into total;
CLOSE c1;
...
--call GPSQLWEB.procCountCgmaxmin('','',0,0,0,0,'','','','','','','','','','','','','','','','','','','','', '', '', '')
