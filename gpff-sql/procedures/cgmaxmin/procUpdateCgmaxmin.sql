...
-- DROP PROCEDURE GPSQLWEB.procUpdateCgmaxmin
    
CREATE PROCEDURE GPSQLWEB.procUpdateCgmaxmin
(
    IN P_ROWID BIGINT,
...
BEGIN
    ...
    SELECT COUNT(1) INTO V_CANT FROM GPDATWEB.CGMAXMIN WHERE RRN(GPDATWEB.CGMAXMIN) =  P_ROWID ;

    IF V_CANT > 1 THEN
          SET P_MSGCODE = 9001;
          SET StringSQL = '9001 - HAY MAS DE UN REGISTRO CON LA CLAVE PRIMARIA';
          CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Actualizar', 'procUpdateCgmaxmin', StringSQL);
          RETURN;
    END IF;
  ...
  Set StringSQL = 'UPDATE GPDATWEB.CGMAXMIN SET CGMEMP= '''||P_CGMEMP || ''' , CGMDEL= '''||P_CGMDEL || ''' , CGMFMT= '''| |P_CGMFMT || ''' , CGMNCT= '''||P_CGMNCT || ''' , CGMCLI= '''||P_CGMCLI || ''' , CGMCOD= '''||P_CGMCOD || ''' , CGMMAX= '''||P_CGMMAX || ''' , CGMMIN= '''||P_CGMMIN || ''' , CGMDS1= '''||P_CGMDS1 || ''' , CGMHS1= '''||P_CGMHS1 || ''' , CGMPR1= '''||P_CGMPR1 || ''' , CGMDS2= '''||P_CGMDS2 || ''' , CGMHS2= '''||P_CGMHS2 || ''' , CGMPR2= '''||P_CGMPR2 || ''' , CGMDS3= '''||P_CGMDS3 || ''' , CGMHS3= '''||P_CGMHS3 || ''' , CGMPR3= '''||P_CGMPR3 || ''' , CGMDS4= '''||P_CGMDS4 || ''' , CGMHS4= '''||P_CGMHS4 || ''' , CGMPR4= '''||P_CGMPR4 || ''' , CGMDS5= '''||P_CGMDS5 || ''' , CGMHS5= '''||P_CGMHS5 || ''' , CGMPR5= '''||P_CGMPR5 || ''' , CGMDS6= '''||P_CGMDS6 || ''' , CGMHS6= '''||P_CGMHS6 || ''' , CGMPR6= '''||P_CGMPR6 || '''  WHERE RRN(GPDATWEB.CGMAXMIN) = '''|| P_ROWID || ''';';
  CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Actualizar', 'procUpdateCgmaxmin', StringSQL);

  UPDATE GPDATWEB.CGMAXMIN SET CGMEMP= P_CGMEMP , CGMDEL= P_CGMDEL, CGMFMT= P_CGMFMT , CGMNCT= P_CGMNCT , CGMCLI= P_CGMCLI , CGMCOD= P_CGMCOD , CGMMAX= P_CGMMAX , CGMMIN= P_CGMMIN , CGMDS1= P_CGMDS1 , CGMHS1= P_CGMHS1 , CGMPR1= P_CGMPR1 , CGMDS2= P_CGMDS2 , CGMHS2= P_CGMHS2 , CGMPR2= P_CGMPR2 , CGMDS3= P_CGMDS3 , CGMHS3= P_CGMHS3 , CGMPR3= P_CGMPR3 , CGMDS4= P_CGMDS4 , CGMHS4= P_CGMHS4 , CGMPR4= P_CGMPR4 , CGMDS5= P_CGMDS5 , CGMHS5= P_CGMHS5 , CGMPR5= P_CGMPR5 , CGMDS6= P_CGMDS6 , CGMHS6= P_CGMHS6 , CGMPR6= P_CGMPR6  
    WHERE RRN(GPDATWEB.CGMAXMIN) =  P_ROWID ;
  SET P_MSGCODE = 0;    
