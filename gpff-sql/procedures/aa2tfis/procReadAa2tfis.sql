...
--DROP  PROCEDURE  GPSQLWEB.procReadAa2tfis

CREATE PROCEDURE  GPSQLWEB.procReadAa2tfis (
    IN P_ROWID BIGINT,
  ...
  Declare C1 Cursor With Return For stmt;
    ...
    -- WHERE CLAUSE
        ...
        IF P_ROWID IS NOT NULL AND P_ROWID>0 THEN
            SET WhereClause = WhereClause || ' AND RRN(GPDATWEB.AA2TFIS) =' || P_ROWID ;
        END IF;
    ...
    IF P_FIRSTRESULT > 0 AND P_MAXRESULT > 0 THEN
         Set StringSQL = 'SELECT * FROM ( SELECT ROW_NUMBER() OVER() AS ROW__NUMBER,  RRN(GPDATWEB.AA2TFIS) as ROWID,FISEMP ,FISDEL ,FISCLA ,FISNOR ,FISBEL ,FISBE1 ,FISDIV ,FISINV ,FISGIN ,FISCAP ,FISRDI ,FISRD1 ,FISCOR ,FISIVA  FROM GPDATWEB.AA2TFIS  WHERE 1=1 ' || WhereClause || ' ) AS LIMITED__TABLE WHERE ROW__NUMBER BETWEEN '|| P_FIRSTRESULT || ' and '|| ((P_MAXRESULT+P_FIRSTRESULT) -1) || SortClause ;
    ELSE
        IF P_MAXRESULT > 0 THEN
            Set StringSQL = 'SELECT   RRN(GPDATWEB.AA2TFIS) as ROWID,FISEMP ,FISDEL ,FISCLA ,FISNOR ,FISBEL ,FISBE1 ,FISDIV ,FISINV ,FISGIN ,FISCAP ,FISRDI ,FISRD1 ,FISCOR ,FISIVA  FROM  GPDATWEB.AA2TFIS WHERE 1=1 ' || WhereClause || SortClause || ' FETCH FIRST '|| P_MAXRESULT ||' ROWS ONLY '; 
        ELSE
            Set StringSQL = 'SELECT   RRN(GPDATWEB.AA2TFIS) as ROWID,FISEMP ,FISDEL ,FISCLA ,FISNOR ,FISBEL ,FISBE1 ,FISDIV ,FISINV ,FISGIN ,FISCAP ,FISRDI ,FISRD1 ,FISCOR ,FISIVA  FROM  GPDATWEB.AA2TFIS WHERE 1=1 ' || WhereClause || SortClause; 
        END IF;
    END IF;

    CALL GPSQLWEB.procCreateWebAudit (P_IPADDRESS, P_USERAGENT, P_USERNAME, 'Leer', 'procReadAa2tfis', StringSQL);
...
--call GPSQLWEB.procReadAa2tfis ('','','','','','','','','','','','','','', 1, 10, '', '', '', '')
